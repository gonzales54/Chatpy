      ...
      <UserGuard>
        <div className="flex h-screen w-full flex-col">
          <div className="relative flex items-center border-b border-gray-300 px-6 py-3">
            <h1 className="font-klee font-semibold">
            ...
            <div
              className={`absolute top-10 right-6 z-10 rounded border border-gray-400 bg-white py-2 ${
                isOptionOpen ? "block" : "hidden"
              }`}
            >
              <button className="w-full px-6 py-1 text-sm" onClick={LogOut}>
                Logout
              </button>
            </div>
          </div>
          ...
          <div className="px-10 text-right">
            <p className="mb-4 text-left font-klee text-sm">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <Link
              href={`/${user?.displayName!}/edit`}
              className="font-klee text-xs text-gray-400"
            >
              ユーザー情報編集
            </Link>
          </div>
